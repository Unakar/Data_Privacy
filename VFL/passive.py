...
class LinearPassive:
    ...
    def _mask_grad(self, enc_grad):
        mask = np.random.normal(0, 1.0, enc_grad.shape)  # TODO
        enc_mask = self.cryptosystem.encrypt_vector(mask)
        enc_mask_grad = enc_grad + enc_mask  # TODO
        return enc_mask_grad, mask

    def _unmask_grad(self, mask_grad, mask):
        true_grad = mask_grad - mask  # TODO
        return true_grad
    ...
    def train(self, trainset):
        ...
        for epoch in trange(self.epochs):
            ...
            for batch in range(n_batches):
                ...
                # -----------------------------------------------------------------
                passive_wx = np.dot(self.params, self.x_train[batch_idxes].T)  # TODO
